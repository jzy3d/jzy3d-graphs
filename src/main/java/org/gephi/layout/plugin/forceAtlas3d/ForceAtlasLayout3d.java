...
public class ForceAtlasLayout3d extends AbstractLayout implements Layout {
  ...
  public void goAlgo() {
    ...
    for (Node n : nodes) {
      if (n.getLayoutData() == null
          || !(n.getLayoutData() instanceof ForceVectorNodeLayoutData3d)) {
    ...
    if (isAdjustSizes()) {
      for (Node n1 : nodes) {
        for (Node n2 : nodes) {
          if (n1 != n2) {
            ForceVectorUtils3d.fcBiRepulsor_noCollide(n1, n2,
                getRepulsionStrength() * (1 + graph.getDegree(n1)) * (1 + graph.getDegree(n2)));
          }
        }
      }
    } else {
      for (Node n1 : nodes) {
        for (Node n2 : nodes) {
          if (n1 != n2) {
            ForceVectorUtils3d.fcBiRepulsor(n1, n2,
                getRepulsionStrength() * (1 + graph.getDegree(n1)) * (1 + graph.getDegree(n2)));
          }
        }
      }
    ...
    if (isAdjustSizes()) {
      if (isOutboundAttractionDistribution()) {
        for (Edge e : edges) {
          ...
          bonus *= getWeight(e);
          ForceVectorUtils3d.fcBiAttractor_noCollide(nf, nt,
              bonus * getAttractionStrength() / (1 + graph.getDegree(nf)));
        }
      } else {
        for (Edge e : edges) {
          Node nf = e.getSource();
      ...
      if (isOutboundAttractionDistribution()) {
        for (Edge e : edges) {
          ...
          double bonus = (nf.isFixed() || nt.isFixed()) ? (100) : (1);
          bonus *= getWeight(e);
          ForceVectorUtils3d.fcBiAttractor(nf, nt,
              bonus * getAttractionStrength() / (1 + graph.getDegree(nf)));
        }
      } else {
        for (Edge e : edges) {
    ...
    for (Node n : nodes) {
      ...
      if (!n.isFixed()) {
        double d = 0.0001 + Math
            .sqrt(nLayout.dx * nLayout.dx + nLayout.dy * nLayout.dy + nLayout.dz * nLayout.dz);
        float ratio;
        if (isFreezeBalance()) {
          nLayout.freeze = (float) (getFreezeInertia() * nLayout.freeze + (1 - getFreezeInertia())
              * 0.1 * getFreezeStrength()
              * (Math.sqrt(Math.sqrt((nLayout.old_dx - nLayout.dx) * (nLayout.old_dx - nLayout.dx)
  ...
  public LayoutProperty[] getProperties() {
     ...
     * NbBundle.getMessage(ForceAtlasLayout3d.class, "forceAtlas.inertia.name"), FORCE_ATLAS,
     * "forceAtlas.inertia.name", NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.inertia.desc"), "getInertia", "setInertia"));
     ...
     * FORCE_ATLAS, "forceAtlas.repulsionStrength.name",
     ...
     * FORCE_ATLAS, "forceAtlas.attractionStrength.name",
     ...
     * FORCE_ATLAS, "forceAtlas.maxDisplacement.name", NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.maxDisplacement.desc"), "getMaxDisplacement", "setMaxDisplacement"));
     * properties.add(LayoutProperty.createProperty( this, Boolean.class,
     * NbBundle.getMessage(ForceAtlasLayout3d.class, "forceAtlas.freezeBalance.name"), FORCE_ATLAS,
     * "forceAtlas.freezeBalance.name", NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.freezeBalance.desc"), "isFreezeBalance", "setFreezeBalance"));
     * properties.add(LayoutProperty.createProperty( this, Double.class,
     * NbBundle.getMessage(ForceAtlasLayout3d.class, "forceAtlas.freezeStrength.name"), FORCE_ATLAS,
     * "forceAtlas.freezeStrength.name", NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.freezeStrength.desc"), "getFreezeStrength", "setFreezeStrength"));
     * properties.add(LayoutProperty.createProperty( this, Double.class,
     * NbBundle.getMessage(ForceAtlasLayout3d.class, "forceAtlas.freezeInertia.name"), FORCE_ATLAS,
     * "forceAtlas.freezeInertia.name", NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.freezeInertia.desc"), "getFreezeInertia", "setFreezeInertia"));
     * properties.add(LayoutProperty.createProperty( this, Double.class,
     * NbBundle.getMessage(ForceAtlasLayout3d.class, "forceAtlas.gravity.name"), FORCE_ATLAS,
     * "forceAtlas.gravity.name", NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.gravity.desc"), "getGravity", "setGravity"));
     * properties.add(LayoutProperty.createProperty( this, Boolean.class,
     * NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.outboundAttractionDistribution.name"), FORCE_ATLAS,
     * "forceAtlas.outboundAttractionDistribution.name",
     * NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.outboundAttractionDistribution.desc"), "isOutboundAttractionDistribution",
     * "setOutboundAttractionDistribution")); properties.add(LayoutProperty.createProperty( this,
     * Boolean.class, NbBundle.getMessage(ForceAtlasLayout3d.class, "forceAtlas.adjustSizes.name"),
     * FORCE_ATLAS, "forceAtlas.adjustSizes.name", NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.adjustSizes.desc"), "isAdjustSizes", "setAdjustSizes"));
     * properties.add(LayoutProperty.createProperty( this, Double.class,
     * NbBundle.getMessage(ForceAtlasLayout3d.class, "forceAtlas.speed.name"), FORCE_ATLAS,
     * "forceAtlas.speed.name", NbBundle.getMessage(ForceAtlasLayout3d.class,
     * "forceAtlas.speed.desc"), "getSpeed", "setSpeed")); } catch (Exception e) {
     * e.printStackTrace(); }
     */
