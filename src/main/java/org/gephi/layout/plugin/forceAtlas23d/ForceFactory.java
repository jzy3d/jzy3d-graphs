...
public class ForceFactory {
  ...
  public AttractionForce buildAttraction(boolean logAttraction, boolean distributedAttraction,
      boolean adjustBySize, double coefficient) {
    if (adjustBySize) {
      if (logAttraction) {
  ...
  public abstract class AttractionForce {
    ...
    public abstract void apply(Node n1, Node n2, double e); // Model for node-node attraction (e is
                                                            // for edge weight if needed)
  }
  ...
  private class linRepulsion extends RepulsionForce {
    ...
    public void apply(Node n1, Node n2) {
      ForceAtlas23dLayoutData n1Layout = n1.getLayoutData();
      ForceAtlas23dLayoutData n2Layout = n2.getLayoutData();

      // Get the distance
      double xDist = n1.x() - n2.x();
      double yDist = n1.y() - n2.y();
      double zDist = n1.z() - n2.z();
      double distance = (float) Math.sqrt(xDist * xDist + yDist * yDist + zDist * zDist);
  ...
  private class linRepulsion_antiCollision extends RepulsionForce {
    ...
    public void apply(Node n1, Node n2) {
      ...
      double zDist = n1.z() - n2.z();
      double distance =
          Math.sqrt(xDist * xDist + yDist * yDist + zDist * zDist) - n1.size() - n2.size();
