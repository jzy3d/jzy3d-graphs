...
public class GephiController {
  ...
  public static void layoutAuto(GraphModel graphModel) {
    ...
    AutoLayout.DynamicProperty adjustBySizeProperty =
        AutoLayout.createDynamicProperty("Adjust by Sizes", Boolean.TRUE, 0.1f);// True after 10% of
                                                                                // layout time
    AutoLayout.DynamicProperty repulsionProperty =
        AutoLayout.createDynamicProperty("Repulsion strength", new Double(500.), 0f);// 500 for the
                                                                                     // complete
                                                                                     // period
  ...
  public void exportPdf(String file) throws IOException {
     ...
     * pdfExporter.setPageSize(PageSize.A0); pdfExporter.setWorkspace(workspace);
     * ByteArrayOutputStream baos = new ByteArrayOutputStream(); ec.exportStream(baos, pdfExporter);
  ...
  public void printInfo() {
    ...
    System.out.println(
        "Nodes: " + directedGraph.getNodeCount() + " Edges: " + directedGraph.getEdgeCount());

    // Get a UndirectedGraph now and count edges
    // UndirectedGraph undirectedGraph = graph.getUndirectedGraph();
    // System.out.println("Edges: "+undirectedGraph.getEdgeCount()); //The mutual edge is
  ...
  public GraphModel editCurrentGraph() {
    ...
    /*
     * for(Node n : directedGraph.getNodes().toArray()) { directedGraph.removeNode(n); }
     */
