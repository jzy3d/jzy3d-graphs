...
/**
 * Renders the graph according to visual settings enables interactive settings of the chart (mouse,
 * keyboard, etc). Listen for graph layout change to update display Support peeking facilities (TODO
 * add newMousePicking())
 *
 */
public class GraphRenderer implements IOnStepDoneListener, IGraphRenderer, IObjectPickedListener {
  ...
  public static GraphRenderer create(GraphModel g, GraphRendererSettings settings, Quality quality,
      String wt, String chart) {
     ...
     * 
     * @Override public void viewWillRender(ViewLifecycleEvent e) { System.out.println("alpha");
     * updatePointAlpha(); }
     * 
  ...
  public GraphRenderer(GraphModel g, GraphRendererSettings settings, Chart c) {
    ...
    /*
     * c.getFactory().ne c.addMouseController(); c.addScreenshotKeyController();
     * c.addKeyController();
     */
  }
  ...
  /**
   * Step ratio is the number of layout steps required to update the chart. Sayed differently, if R
   * is number of rendering occurence, and L number of layout steps, the resulting referesh ratio is
   * R/L.
   * 
   * @param stepRatio
   */
  ...
  protected void createNode(Node n) {
    ...
    if (settings.isNodeSphereDisplayed()) {
      Sphere s = createNodeSphere(center, settings.getNodeColor(n),
          settings.getNodeWireframeColor(n), radius, 15);
  ...
  private Coord3d createCoordinate(Node nd) {
    return new Coord3d(Float.isNaN(nd.x()) ? 0 : nd.x(), Float.isNaN(nd.y()) ? 0 : nd.y(),
  ...
  protected void createEdge(Edge e) {
    ...
    LineStrip ls =
        createEdgeLine(edgeCoordinates.a, edgeCoordinates.b, settings.getEdgeColorSource(e), 1);
    addToSceneGraph(ls);
  ...
  protected void addToSceneGraph(Drawable s) {
  ...
  /*
   * public NewtMousePickingController<Node,String> getChartMouseController(){ return
   * (NewtMousePickingController<Node,String>)
   * ((CanvasAWT)getChart().getCanvas()).getMouseListeners()[0]; }
   * 
   * public PickingSupport getPickingSupport(){ try{ return
   * getChartMouseController().getPickingSupport(); } catch(ClassCastException e){ return null; } }
   * 
   * public void registerNodeRepresentationPicking(Node n){ Sphere s = getNodeRepresentation(n);
   * PickingSupport sp = getPickingSupport(); if(sp!=null) sp.registerDrawableObject(s, n); }
   */
  ...
  protected Map<Object, DrawableTextWrapper> nodeLabelRepresentation =
      new HashMap<Object, DrawableTextWrapper>();
